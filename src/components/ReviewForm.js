...
function checkInputParam(titles, inputParams) {
	if (inputParams.reviewPoint1 === 0) {
		return { result: false, errorMessage: titles.reviewPoint1Title + "Пожалуйста выберите" };
	} else if (inputParams.reviewPoint2 === 0) {
		return { result: false, errorMessage: titles.reviewPoint2Title + "Пожалуйста выберите" };
	} else if (inputParams.reviewPoint3 === 0) {
		return { result: false, errorMessage: titles.reviewPoint3Title + "Пожалуйста выберите" };
	} else if (inputParams.overAllPoints === 0) {
		return { result: false, errorMessage: titles.overAllPointsTitle + "Пожалуйста выберите" };
	} else if (inputParams.motivationFreeText.length > 100) {
		return { result: false, errorMessage: titles.motivationFreeTextTitle + "Вы должны ввести не более 100 символов." };
	} else if (inputParams.freeWriting.length > 800) {
		return { result: false, errorMessage: titles.freeWritingTitle + "Вы должны ввести не более 800 символов." };
	} else {
